...
"""
modified from plot.py in OverSim
...
parser = OptionParser(usage="%prog [options] yscalarname *.sca")
...
# Example:
# 
# ./scalar-extract.py delayOneOverlayHop:mean -c 0.95 /nix/workspace/omnet422-inet118/store/sim-results/wMathias/delays/delays-500n-srate500kbps-upbw2500kbps-5partners/*.sca
...
#parser.add_option("-S", "--offset", type="float", default=0, helpa="Add OFFSET to y values")
...
for infilename in args[1:]:
    ...
    if not yscalarFound:
        print "Error: y scalar \"" + args[0] + "\" not found in " + oldInfile + "!"
        exit(-1)
    ...
    for line in infile:
        if parseHeader:
            ...
            # find xvar
            if line.startswith("attr " + xvarname + " "):
                xvarFound = True
                temp = line.split(" ", 3)[2]
                #print "temp = " + temp # <-- debug
                bucket = float(len(temp))
                #bucket = float(temp.split("=",1)[1])
...
for run in sorted(valuemap.keys()):
    ...
    for bucket, bucketdata in rundata.iteritems():
        ...
        if options.ci > 0:
            ...
            bucketci = stats.sem(bucketarray) * stats.t._ppf((1+options.ci)/2., len(bucketarray))
            ci[bucket] = bucketci
