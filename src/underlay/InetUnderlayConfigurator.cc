...
void InetUnderlayConfigurator::initialize(int stage) {
    ...
    if (stage == 2) {
        ...
        scheduleAt(simTime(), new cMessage("CreateAllNodes"));
...
void InetUnderlayConfigurator::handleMessage(cMessage* msg) {
    ...
    if(strcmp(msg->getName(),"CreateAllNodes") == 0) {
		...
		// when all potential ALM routers are deployed. This needs to be done before printLiveStart is happening, which is evaluating the actual ALM router positions.
		kfournisseur();
...
cModule* InetUnderlayConfigurator::createNode(bool conn) {
	...
	if (numSources < maxNumSources) {
        ...
        terminal = moduleType->create("cli", getParentModule(), numNodes, numTerminals);
...
void InetUnderlayConfigurator::getAccessRouter() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (getParentModule()->findSubmodule("rt"), i) {
			...
			DEBUGOUT("1.router->gateSize(out): rt["<<i<<"] :"<<router->gateSize("out"));
            // the ned file should not contain unconnected routers
            assert(router->gateSize("out") != 0);

			if (router->gateSize("out") == 1 || routerLine)
            	accessRouter.push_back(router);

            if (!(bool)router->par("isTransit")) {
                sum += 1/(double)router->gateSize("out");
