...
{
   Enter_Method("setRoute");

   assert(!route.empty());
   EV << "route is NOT empty!" << endl;
   routes[addr] = route;

   // Print received Route
   EV << getParentModule()->getParentModule()->getParentModule()->getFullName() << " Dest: " << addr << endl;
   int size = route.size();
   EV << "size = " << size << endl;
   for (int i = 0; i !=size; i++)
   {
      EV << route[i]->hop << " " << route[i]->addr << endl;
   }
...
bool TracerouteTable::inTable(IPvXAddress addr)
{
   if (routes.find(addr) == routes.end())
      return false;
